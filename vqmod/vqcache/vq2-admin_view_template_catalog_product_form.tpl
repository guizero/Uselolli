          ...
          <div id="tab-option-<?php echo $option_row; ?>" class="vtabs-content">
            ...
            <table id="option-value<?php echo $option_row; ?>" class="list">
              ...
              <tfoot>
                <tr>
                  <?php if ($product_option['type'] == 'select') { ?>
                  <td colspan="<?php $i = 6; (int)$i++; echo $i; ?>"></td>
                  <?php } else { ?>
                  <td colspan="6"></td>
                  <?php } ?>
                  <td class="left"><a onclick="addOptionValue('<?php echo $option_row; ?>', '<?php echo $product_option['type']; ?>');" class="button"><?php echo $button_add_option_value; ?></a></td>
                </tr>
              </tfoot>
            </table>
...
$('input[name=\'option\']').catcomplete({
	...
	select: function(event, ui) {
		...
		if (ui.item.type == 'select' || ui.item.type == 'radio' || ui.item.type == 'checkbox' || ui.item.type == 'image') {
			...
			html += '      <tr>';
			if (ui.item.type == 'select') {
			html += '        <td colspan="<?php $i = 6; (int)$i++; echo $i; ?>"></td>';
			} else {
			html += '        <td colspan="6"></td>';
			}
			html += '        <td class="left"><a onclick="addOptionValue(\'' + option_row + '\', \'' + ui.item.type + '\');" class="button"><?php echo $button_add_option_value; ?></a></td>';
			html += '      </tr>';
			html += '    </tfoot>';
			html += '  </table>';
